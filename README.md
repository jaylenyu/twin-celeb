# Twin Celeb
 ...
 - **자동 압축 (Web Worker)** — 메인 스레드 차단을 방지하는 Web Worker 기반 이미지 압축
 ...
 - **API 응답 속도**: Web Worker 기반 이미지 압축으로 API 토큰 절감
 ...
 └── layout.tsx
 components/
 ├── ImageUpload.tsx            # 업로드 UI (Web Worker 압축)
 ...
 __tests__/                     # 테스트 코드
 workers/
 └── imageCompression.worker.ts # Web Worker 이미지 압축
 ```
